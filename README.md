...
ai engineer building llm-powered products — production rag pipelines with real evals,
...
- **isra** — hybrid-retrieval rag over the indian startup ecosystem. pgvector + postgres full-text
  fused via reciprocal rank fusion, bge cross-encoder reranking (no langchain), llm-as-judge evals
  (mrr 0.688 → 0.750), streaming sse chat with grounded citations, langfuse tracing.
  [live](https://isra-web-gamma.vercel.app/) · [repo](https://github.com/prayagtushar/isra)

- **multi-llm client** — unified async python client for openai, anthropic & gemini.
  one interface for messages, streaming, usage, and errors; ships as library, cli, repl,
  and fastapi service. pydantic v2, mypy-strict, 40 tests.
...
open to interesting problems, especially around ai infra and llm tooling.
